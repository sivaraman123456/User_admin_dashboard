...
for (let i = 3; i <= 50; i++) {
  const name = `${names[i % names.length]} ${["Lee", "Chen", "Garcia", "Patel", "Khan", "Brown", "O'Neil"][i % 7]}`
  USERS.push({
    id: i,
    name,
    ...
    last_login: new Date(Date.now() - Math.random() * 1000 * 60 * 60 * 24 * 30).toISOString(),
...
export async function listUsers(opts: {
  ...
  if (opts.sortBy) {
    filtered = filtered.slice().sort((a, b) => {
      ...
      return opts.sortDir === "asc" ? Number(av) - Number(bv) : Number(bv) - Number(av)
    })
  }
