...
export default function UserDetail({ id }: { id: number }) {
  ...
  const { data, isLoading, mutate, error } = useSWR<UserDetailResponse>(`/api/users/${id}`, fetcher)
  const {
    data: txs,
    isLoading: txLoading,
    error: txError,
  } = useSWR<{ items: UserDetailResponse["transactions"] }>(`/api/users/${id}/transactions`, fetcher)
  ...
  return (
    ...
    <div className="flex items-center justify-between flex-1">
      <h1 className="text-xl font-semibold text-pretty">User Detail: {data.user.name}</h1>
      <Link href="/users">
...
<Card className="flex-1 ">
      ...
      <CardContent className="grid md:grid-cols-2 gap-3">
        ...
        <div className="md:col-span-2 flex items-center gap-3">
          ...
          <Select value={data.user.kyc_status} onValueChange={(val) => updateKyc(val as KycStatus)} disabled={saving}>
    ...
    <div className="grid md:grid-cols-3 gap-3 flex-1 items-center">
      {data.balances.map((b) => (
        <Card key={b.asset}>
          ...
          <CardContent>
            <div className="text-2xl font-semibold tabular-nums">{b.amount}</div>
          </CardContent>
        </Card>
      ))}
    </div>
    ...
    <Card>
      ...
      <CardContent>
        ...
        {txError && <div className="text-muted-foreground">Failed to load transactions.</div>}
        {!txLoading && !txError && (
                    ...
                    <tr key={t.id} className="border-t">
                      ...
                      <td className="px-3 py-2">
                        <Badge
                          ...
                          className={`${t.status === "Completed" ? 'bg-green-500' : '' }`}
                        >
                          {t.status}
                        </Badge>
                      </td>
